...
def get_latest_folder(base_path):
    ...
    folder_list = sorted(
        folders, key=lambda f: datetime.strptime(os.path.basename(f), "%Y%m%d_%H%M%S")
    )
...
def calculate_client_scores(
    clients_uploads, coefficient, weight_size=2, total_rewards=100
):
    client_scores = {}
    ...
    client_score_percentages = {
        client_id: round(score / total_score, 4) * total_rewards * coefficient
