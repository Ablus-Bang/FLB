...
def deserialize_model_state_dict(serialized_state_dict):
    ...
    for key, byte_tensor in serialized_state_dict.items():
        ...
        tensor = torch.load(
            buffer, weights_only=True, map_location=torch.device(device)
        )
...
class WeightsTransferServicer(communicate_pb2_grpc.WeightsTransferServicer):
    ...
    def SendWeights(self, request, context):
        if request.HasField("send_parameters"):
            ...
            print(
                f"Received weights from client {client_id} with dataset length {train_dataset_length}"
            )
            lora_data = dict()
            for lora_config in lora_config:
                lora_data[lora_config.config_name] = parse_value(
                    lora_config.config_value
                )
            ...
            return communicate_pb2.TransferStatus(
                code=True, message="Weights received successfully"
            )
        else:
            return communicate_pb2.TransferStatus(
                code=False, message="Invalid message type"
            )
