...
class DpServerFixedClip(Strategy):
    ...
    def aggregate(
        ...
        for k, p in enumerate(weight_path_list):
            ...
            single_weights = torch.load(
                single_output_dir, map_location=torch.device(self.device_map)
            )
            single_weights, _ = clip_l2_norm(
                single_weights,
                self.params_current,
                self.clip_threshold,
                self.device_map,
        ...
        new_weight = self.strategy.aggregate(
