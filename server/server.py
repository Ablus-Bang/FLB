...
class BaseServer:
    def __init__(self, cfg_path: str, strategy: Strategy = None):
        ...
        if self.latest_version is None:
            latest_time = (datetime.today() - timedelta(days=1)).strftime(
                "%Y%m%d_%H%M%S"
            )
            output_path = os.path.join(self.output, latest_time)
    ...
    def aggregate(self, client_list, dataset_len_list, weight_path_list):
        self.model_parameter = self.strategy.aggregate(
            client_list, dataset_len_list, weight_path_list
        )

    def save_model(self):
        new_version = datetime.today().strftime("%Y%m%d_%H%M%S")
        save_path = os.path.join(self.output, new_version)
    ...
    def calculate_reward(self, clients):
        if self.latest_version != self.pre_version:
            latest_result_path = os.path.join(
                self.output, self.latest_version, "eval_result.json"
            )
            pre_result_path = os.path.join(
                self.output, self.pre_version, "eval_result.json"
            )
            ...
            coefficient = (
                min(0, latest_result["total"]["accu"] - pre_result["total"]["accu"])
                / latest_result["total"]["accu"]
                if latest_result["total"]["accu"] > 0
                else 0
            )
        else:
            coefficient = 1
    ...
    def eval(
        self, lora_config_path="", model_weights_path="", clients_data_detail=None
    ):
        results = eval_model(
            self.config_detail.model.model_path,
            lora_config_path,
            model_weights_path,
            n_train=1,
        )
    ...
    def update(self, do_eval=True):
        ...
        clients_detail, dataset_length_list, path_list = get_clients_uploads_after(
            self.save_path, self.latest_version
        )
        ...
        if len(client_list) > 0:
            ...
            self.save_model()
            if do_eval:
                weight_saved_path = os.path.join(
                    self.output, self.latest_version, "adapter_model.bin"
                )
                eval_thread = Thread(
                    target=self.eval,
                    args=["./output", weight_saved_path, clients_detail],
                )
                eval_thread.start()
        else:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            for _ in range(self.num_clients):
                ...
                with conn:
                    ...
                    with open(
                        client_save_path + "/train_dataset_length.json", "w"
                    ) as f:
                        json.dump(
                            {"train_dataset_length": recv_data["train_dataset_length"]},
                            f,
                        )
    ...
    def run_grpc_server(self):
        ...
        grpc_server = grpc.server(
            futures.ThreadPoolExecutor(max_workers=10), options=channel_options
        )
        communicate_pb2_grpc.add_WeightsTransferServicer_to_server(
            WeightsTransferServicer(self), grpc_server
        )
